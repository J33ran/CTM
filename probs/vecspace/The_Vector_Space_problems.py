...
from hw.vec.vec import Vec
from gf2.gf2 import one,zero
...
def GF2_span(D, S):
    ...
    True
    '''    
    #vec_span = {Vec(D, {d:0 for d in D})}
    vec_span = {Vec(D, {})}

    for v in S:
        for v_1 in {one*v, zero*v}:
            vec_list = [v + v_1 for v in vec_span] 
            vec_span = vec_span |  set(vec_list)
